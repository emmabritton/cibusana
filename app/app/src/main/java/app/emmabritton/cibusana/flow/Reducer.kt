...
import app.emmabritton.cibusana.flow.home.reduceHomeAction
...
import app.emmabritton.cibusana.flow.measurements.reduceMeasurementAction
...
import app.emmabritton.cibusana.flow.weight.reduceWeightAction
...
fun reduce(action: Action, state: AppState): AppEffect {
    return when (action) {
        ...
        is HomeAction -> reduceHomeAction(action, state)
        is MeasurementAction -> reduceMeasurementAction(action, state)
        is CommandException -> {
