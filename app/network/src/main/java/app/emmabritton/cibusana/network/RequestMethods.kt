...
fun <R> executeOptionalRequest(logger: Logger, call: Call<ResponseWrapper<R>>): Result<R?> {
    return try {
...
fun <R> executePagedRequest(logger: Logger, call: Call<ResponseWrapper<PageWrapper<R>>>): Result<Pair<Int,List<R>>> {
    return try {
        val response = call.execute()
        if (response.isSuccessful) {
            val body = response.body()
            if (body == null || (body.content == null && body.error == null)) {
                ...
                if (body.content != null) {
                    Result.success(Pair(body.content.page, body.content.items))
                } else {
                    logger.e("Bad request (${body.error!!.codes}) for $call")
                    Result.failure(BadRequestException(body.error.codes, body.error.message))
                }
